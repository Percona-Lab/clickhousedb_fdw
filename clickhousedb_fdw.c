...
static void clickhouseEndForeignInsert(EState *estate,
						 ResultRelInfo *resultRelInfo);
static void clickhouseExplainForeignScan(ForeignScanState *node, ExplainState *es);
static void clickhouseGetForeignUpperPaths(PlannerInfo *root, UpperRelationKind stage,
...
static void prepare_foreign_modify(TupleTableSlot *slot,CHFdwModifyState *fmstate);
						 ...
						 TupleTableSlot *slot)
{
	...
	elog(DEBUG2, "> %s:%d   %s:%p", __FUNCTION__, __LINE__, fmstate->conn->query, fmstate->conn->query);
	initStringInfo(&sql);
	...
	if (slot != NULL && fmstate->target_attrs != NIL)
	{
        ...
        break;
			    ...
			    float8 f = DatumGetFloat8(valueDatum);
        	if (!first)
        	  appendStringInfo(&sql, ", ");
      		first = false;
					appendStringInfo(&sql, "%f", f);
        ...
        case TIMESTAMPOID:
      		...
      		first = false;
      		timestamp2tm(valueTimestamp, &tz, tm, &fsec, &tzn, pg_tzset("UTC"));
          appendStringInfo(&sql, "'%04d-%02d-%02d %02d:%02d:%02d'", tt.tm_year, tt.tm_mon, tt.tm_mday, tt.tm_hour, tt.tm_min, tt.tm_sec);
          break;
				}
				default:
				{
							 ...
							 char *query)
{
	...
	foreach(lc, retrieved_attrs)
	{
		...
		char	  *valstr;
		int     len = 256;
		char    *str = (char*)palloc(256); /* Hard limit for the string, need to be fixed. */
		bool    is_null = false;
